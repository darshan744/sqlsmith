...
shared_ptr<table_ref> table_ref::factory(prod* p) {
    try {
        if (p->level < 3 + d6()) {
            ...
            if (d6() > 3) return make_shared<joined_table>(p);
        }
        if (d6() > 3)
...
expr_join_cond::expr_join_cond(prod* p, table_ref& lhs, table_ref& rhs)
    ...
    scope = &joinscope;
    for (auto ref : lhs.refs) joinscope.refs.push_back(&*ref);
...
joined_table::joined_table(prod* p) : table_ref(p) {
    ...
    for (auto ref : lhs->refs) refs.push_back(ref);
...
struct for_update_verify : prod_visitor {
    virtual void visit(prod* p) {
        if (dynamic_cast<window_function*>(p)) throw("window function");
        joined_table* join = dynamic_cast<joined_table*>(p);
        if (join && join->type != "inner") throw("outer join");
        query_spec* subquery = dynamic_cast<query_spec*>(p);
        if (subquery) subquery->set_quantifier = "";
        ...
        if (tab) {
            ...
            if (actual_table && !actual_table->is_insertable)
                throw("read only");
            if (actual_table->name.find("pg_")) throw("catalog");
        }
...
void common_table_expression::accept(prod_visitor* v) {
    v->visit(this);
    for (auto q : with_queries) q->accept(v);
    query->accept(v);
}
...
merge_stmt::merge_stmt(prod* p, struct scope* s, table* v)
    ...
    while (d6() > 4) clauselist.push_back(when_clause::factory(this));
}
...
void group_by::make_combo(
    ...
    for (int i = currentIndex; i < cols.size(); i++) {
