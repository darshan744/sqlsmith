...
target_table::target_table(prod *p, table *victim) : table_ref(p) {
  while (!victim || victim->schema == "pg_catalog" || !victim->is_base_table ||
         !victim->columns().size()) {
...
void group_by::make_combo(int currentIndex, int perGroupCount,
                          std::vector<column> &cols,
                          std::vector<std::vector<column>> &result,
                          std::vector<column> temporaryColumnHolder) {
...
group_by::group_by(prod *p, shared_ptr<select_list> select_list) : prod(p) {
  ...
  /// we choose a subset of the selected columns randomly (might also have all
  /// the columns);
...
void group_by::printSimpleGroup(std::ostream &out, std::vector<column> col) {
  out << "( ";
...
void group_by::out(std::ostream &out) {
  out << "GROUP BY ";
  if (isSimpleGroupBy) {
    return printSimpleGroup(out, group_by_cols[0]);
  }
  out << "( ";
  for (int i = 0; i < group_by_cols.size(); i++) {
    printSimpleGroup(out, group_by_cols[i]);
    if (i + 1 != group_by_cols.size()) {
      out << ",\n";
    }
  }
  out << " )";
