...
impedance_visitor::~impedance_visitor() {
    for (auto pair : found) _occured[pair.first]++;
}
...
bool matched(const char *name) {
    if (100 > occurances_in_failed_query[name]) return true;
    double error_rate =
...
void report() {
    ...
    for (auto pair : occurances_in_failed_query) {
        ...
        if (!matched(pair.first)) cerr << " -> BLACKLISTED";
        cerr << endl;
    }
}
...
void report(std::ostream &out) {
    ...
    for (auto pair = occurances_in_failed_query.begin();
        ...
        if (next(pair) != occurances_in_failed_query.end()) out << "," << endl;
    }
    out << "]}" << endl;
}
