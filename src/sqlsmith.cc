...
extern "C" void cerr_log_handler(int) {
    if (global_cerr_logger) global_cerr_logger->report();
    exit(1);
}
...
int main(int argc, char *argv[]) {
    ...
    regex optregex(
        "--(help|log-to|verbose|target|sqlite|monetdb|version|dump-all-graphs|"
        "dump-all-queries|seed|dry-run|max-queries|rng-state|exclude-catalog)(?"
        ":=((?:.|\n)*))?");
