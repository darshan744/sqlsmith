...
void monetdb_connection::q(const char *query) {
    ...
    if (mapi_result_error(hdl) != NULL) mapi_explain_result(hdl, stderr);
    mapi_close_handle(hdl);
}

// disconnect MonetDB
monetdb_connection::~monetdb_connection() { mapi_destroy(dbh); }

// load schema from MonetDB
schema_monetdb::schema_monetdb(std::string &conninfo)
    : monetdb_connection(conninfo) {
    ...
    //	string qry = "select t.name, s.name, t.system, t.type from sys.tables t,
    //sys.schemas s where t.schema_id=s.id and t.system=false";
    string qry =
        "select t.name, s.name, t.system, t.type from sys.tables t,  "
        "sys.schemas s where t.schema_id=s.id ";
    MapiHdl hdl = mapi_query(dbh, qry.c_str());
    while (mapi_fetch_row(hdl)) {
        tables.push_back(
            table(mapi_fetch_field(hdl, 0), mapi_fetch_field(hdl, 1),
    ...
    for (auto t = tables.begin(); t != tables.end(); t++) {
        ...
        while (mapi_fetch_row(hdl)) {
            column c(mapi_fetch_field(hdl, 0),
                     sqltype::get(mapi_fetch_field(hdl, 1)));
    ...
    string opq(
        ...
        "  where f.id=a.func_id and f.id=b.func_id and f.id=c.func_id and "
        "a.name='arg_1' and b.name='arg_2' and c.number=0");
    hdl = mapi_query(dbh, opq.c_str());
    while (mapi_fetch_row(hdl)) {
        op o(mapi_fetch_field(hdl, 0), sqltype::get(mapi_fetch_field(hdl, 1)),
             sqltype::get(mapi_fetch_field(hdl, 2)),
             sqltype::get(mapi_fetch_field(hdl, 3)));
        register_operator(o);
    }
    mapi_close_handle(hdl);
    cerr << " done." << endl;
    ...
    string routq(
        "select s.name, f.id, a.type, f.name from sys.schemas s, sys.args a, "
        "sys.types t, sys.functions f where f.schema_id = s.id and "
        "f.id=a.func_id and a.number=0 and a.type = t.sqlname and "
        "f.mod<>'aggr'");
    hdl = mapi_query(dbh, routq.c_str());
    while (mapi_fetch_row(hdl)) {
        routine proc(mapi_fetch_field(hdl, 0), mapi_fetch_field(hdl, 1),
                     sqltype::get(mapi_fetch_field(hdl, 2)),
    ...
    /*  	for (auto item : tables) {
            cerr << item.name << "; " << item.schema << "; " <<
       item.is_insertable << "; " << item.is_base_table << endl;
        }
    */
    //	cerr << "Loaded columns... " << endl;
    /*  	for (auto tab : tables) {
            for (auto col: tab.columns())
                cerr << tab.name << "; " << col.name << "; "<<col.type->name <<
       endl;
        }
    */
    //	cerr << "Loaded aggregates and parameters... " << endl;
    /* 	for (auto &proc : aggregates) {
            cerr << proc.specific_name << "; " << proc.schema << "; " <<
       proc.name <<"; " << proc.restype->name ; for (auto item : proc.argtypes)
                cerr << "; " << item->name;
            cerr << endl;
        }
    */
...
void dut_monetdb::test(const std::string &stmt) {
    ...
    if (mapi_error(dbh) != MOK) {
        try {
            ...
            if (!error_string) error_string = "unknown error";

            const char *sqlstate = mapi_result_errorcode(hdl);
            if (!sqlstate) sqlstate = "XXXXX";
