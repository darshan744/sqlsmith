        ...
        case 'p': /* pseudo type: accept any concrete matching type */
            if (name == "anyarray" || name == "anycompatiblearray") {
                ...
                return t->typtype_ == 'e';
                       ...
                       name ==
                           "anycompatible") { /* as quoted by quote_ident() */
...
schema_pqxx::schema_pqxx(std::string &conninfo, bool no_catalog) : c(conninfo) {
    ...
    r = w.exec(
        "select case typnamespace when 'pg_catalog'::regnamespace then "
        "quote_ident(typname) "
...
void dut_libpq::connect(std::string &conninfo) {
    ...
    if (PQstatus(conn) != CONNECTION_OK) {
        ...
        if (strlen(errmsg)) throw dut::broken(errmsg, "08001");
    }
...
void dut_libpq::command(const std::string &stmt) {
    ...
    switch (PQresultStatus(res)) {
        ...
        default: {
            ...
            if (!errmsg || !strlen(errmsg)) errmsg = PQerrorMessage(conn);

            const char *sqlstate = PQresultErrorField(res, PG_DIAG_SQLSTATE);
            if (!sqlstate || !strlen(sqlstate))
                sqlstate =
                    (CONNECTION_OK != PQstatus(conn)) ? "08000" : "?????";
            ...
            if (CONNECTION_OK != PQstatus(conn)) {
                ...
                throw dut::broken(error_string.c_str(),
                                  sqlstate_string.c_str());
            }
            if (sqlstate_string == "42601")
                throw dut::syntax(error_string.c_str(),
                                  sqlstate_string.c_str());
            else
