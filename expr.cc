...
shared_ptr<value_expr> value_expr::factory(prod *p, sqltype *type_constraint) {
  try {
    // allowed only in select statements
...
comparison_op::comparison_op(prod *p) : bool_binop(p) {
  ...
  // equal_range returns a pair of iterators that has same key as the passed
  // one. since map don't have dups, we get that iterator himself.
...
const_expr::const_expr(prod *p, sqltype *type_constraint)
  ...
  if (type == scope->schema->inttype)
    ...
    expr +=
        (d6() > 3) ? scope->schema->true_literal : scope->schema->false_literal;
...
/// It chooses one aggregate function
/// One or more partition by and order by columns
