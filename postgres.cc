...
static regex
    e_timeout("ERROR:  canceling statement due to statement timeout(\n|.)*");
  ...
  case 'p': /* pseudo type: accept any concrete matching type */
    if (name == "anyarray" || name == "anycompatiblearray") {
      ...
      return t->typtype_ == 'e';
               ...
               name == "anycompatible") { /* as quoted by quote_ident() */
...
schema_pqxx::schema_pqxx(std::string &conninfo, bool no_catalog) : c(conninfo) {
  ...
  // address the schema change in postgresql 11 that replaced proisagg and
  // proiswindow with prokind
  string procedure_is_aggregate =
      version_num < 110000 ? "proisagg" : "prokind = 'a'";
  string procedure_is_window =
      version_num < 110000 ? "proiswindow" : "prokind = 'w'";

  cerr << "Loading types...";

  r = w.exec("select case typnamespace when 'pg_catalog'::regnamespace then "
             "quote_ident(typname) "
  ...
  for (auto row = r.begin(); row != r.end(); ++row) {
    ...
    pg_type *t = new pg_type(name, oid, typdelim[0], typrelid, typelem,
                             typarray, typtype[0]);
    oid2type[oid] = t;
    name2type[name] = t;
    types.push_back(t);
  ...
  for (auto row = r.begin(); row != r.end(); ++row) {
    ...
    tables.push_back(table(row[0].as<string>(), schema,
  ...
  for (auto row : r) {
    op o(row[0].as<string>(), oid2type[row[1].as<OID>()],
         oid2type[row[2].as<OID>()], oid2type[row[3].as<OID>()]);
    register_operator(o);
  }
  ...
  r = w.exec("select (select nspname from pg_namespace where oid = "
             "pronamespace), oid, prorettype, proname "
             "from pg_proc "
             "where prorettype::regtype::text not in ('event_trigger', "
             "'trigger', 'opaque', 'internal') "
             "and proname <> 'pg_event_trigger_table_rewrite_reason' "
             "and proname <> 'pg_event_trigger_table_rewrite_oid' "
             "and proname !~ '^ri_fkey_' "
             "and not (proretset or " +
             procedure_is_aggregate + " or " + procedure_is_window + ") ");

  for (auto row : r) {
    routine proc(row[0].as<string>(), row[1].as<string>(),
                 oid2type[row[2].as<long>()], row[3].as<string>());
  ...
  r = w.exec(
      ...
      "from pg_proc "
      "where prorettype::regtype::text not in ('event_trigger', 'trigger', "
      "'opaque', 'internal') "
      ...
      ") "
      "and " +
      procedure_is_aggregate);

  for (auto row : r) {
    routine proc(row[0].as<string>(), row[1].as<string>(),
                 oid2type[row[2].as<OID>()], row[3].as<string>());
...
dut_libpq::dut_libpq(std::string conninfo) : conninfo_(conninfo) {
  connect(conninfo);
}
