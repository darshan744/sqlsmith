...
int main(int argc, char *argv[]) {
  ...
  regex optregex("--(help|log-to|verbose|target|sqlite|monetdb|version|dump-"
  ...
  if (options.count("help")) {
    cerr
        << "    --target=connstr     postgres database to send queries to"
        << endl
        <<
#ifdef HAVE_LIBSQLITE3
        ...
        << "    --seed=int           seed RNG with specified int instead of PID"
        << endl
        ...
        << "    --help               print available command line options and "
           "exit"
        << endl;
    return 0;
...
#else
      cerr << "Sorry, " PACKAGE_NAME " was compiled without SQLite support."
           << endl;
      return 1;
#endif
...
#else
      cerr << "Sorry, " PACKAGE_NAME " was compiled without MonetDB support."
           << endl;
      return 1;
#endif
    } else
          ...
          try {
            ...
            for (auto l : loggers)
              try {
                ...
                cerr << endl
                     << "log failed: " << typeid(*l).name() << ": " << e.what()
                     << endl;
              }
